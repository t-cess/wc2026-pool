...
async function fetchEspn(dateStr) {
  ...
  return (d.events||[]).map(e => {
    ...
    return {
      ...
      date: Date.parse(e.date),                                     // เวลานัด (กำหนดการ ESPN) — ใช้ resync kickoff
...
const W_BEFORE = 5*60*1000, W_GRADE = 8*60*60*1000;
...
// มีคู่ "ยังไม่ตรวจจบ" ในหน้าต่างตรวจ (kickoff ย้อนถึง 8 ชม. = W_GRADE เดียวกับ loop) ไหม
// ใช้ 8 ชม. (ไม่ใช่ 3) กันคู่เตะสาย/ต่อเวลา+จุดโทษ ที่จบหลัง kickoff+3ชม. หลุด gate → ค้าง live ถาวร + ไม่ยิงสรุป
// self-closing: gate ปิดเองเมื่อทุกคู่ในหน้าต่าง finished+autoGraded (คู่เก่ากว่า 8 ชม. หลุด query เอง)
async function hasLiveWindow() {
  for (const md of RD(await matchesInWindow(W_GRADE)).docs) {
    const m = md.data();
...
// 🕐 sync เวลา kickoff คู่ที่ยังไม่เตะจาก ESPN — ตารางเลื่อน/เวลาเก็บผิด ต้องแก้ "ก่อน" ถึงปิดรับ (kickoff-10min) ไม่งั้นปิดรับ/เปิดไพ่ผิดเวลา
// แตะเฉพาะคู่ที่ ESPN ยัง state="pre" (ยังไม่เตะ) → ไม่ขยับคู่ที่กำลังเตะ/จบแล้ว (กันกระทบ grade/digest/กันโกงที่ทำงานไปแล้ว)
// ใช้ array espn (±1 วัน) ที่ flow หลักดึงมาแล้ว = ไม่ fetch ซ้ำ · คู่ไกลกว่านั้นจะถูกแก้เมื่อเข้ากรอบ ±1 วัน (ยังเหลือเวลาก่อน lock อีกเป็นวัน)
async function resyncKickoffs(ms, espn) {
  const SKEW = 5*60*1000;   // ต่าง > 5 นาทีถึงแก้ (กัน churn จากเศษวินาที)
  let fixed = 0;
  for (const m of ms) {
    if (m.status==="finished") continue;
    const ev = espn.find(e => e.home===m.home && e.away===m.away);
    if (!ev || ev.state!=="pre" || !ev.date || Math.abs(ev.date - m.kickoff) <= SKEW) continue;
    console.log(`${DRY?"[DRY] ":""}🕐 resync ${m.home}-${m.away}: ${thKick(m.kickoff)} → ${thKick(ev.date)} (ESPN)`);
    if (!DRY) await matchesCol().doc(m.id).set({ kickoff: ev.date }, {merge:true});
    fixed++;
  }
  if (fixed) console.log(`🕐 resync kickoff ${fixed} คู่`);
}
...
async function nightDigest(recent) {   // recent = คู่ 48 ชม. (แชร์จาก autoAddNext กัน read ซ้ำ)
  ...
  const done = new Set(cfg.digested||[]);
...
async function run() {
  ...
  const doPeriodic = !lowPower && (FORCE || DRY || new Date().getMinutes() % 5 === 0);   // งานราย ~5 นาที: resync kickoff / auto-add / สรุปวัน
  // ดึง ESPN เมื่อวาน+วันนี้+พรุ่งนี้ (กันคาบเกี่ยวเที่ยงคืน) — ใช้ทั้ง grade + resync kickoff · ข้ามถ้า idle (ไม่มีบอลเตะ + ไม่ถึงรอบ periodic) กันยิง ESPN เปล่า
  const now = new Date();
  const ds = (live || doPeriodic) ? [-1,0,1].map(o=>{ const d=new Date(now); d.setDate(d.getDate()+o);
    return d.getFullYear()+String(d.getMonth()+1).padStart(2,"0")+String(d.getDate()).padStart(2,"0"); }) : [];
  const espn = ds.length ? (await Promise.all(ds.map(fetchEspn))).flat() : [];   // เอาทั้ง จบ + สด
  if (espn.length) console.log("ESPN จบ:", espn.filter(e=>e.final).map(e=>`${e.home} ${e.hs}-${e.as} ${e.away}`).join(" | ")||"-",
    "· สด:", espn.filter(e=>e.live).map(e=>`${e.home} ${e.hs}-${e.as} ${e.away} ${e.clock}`).join(" | ")||"-");
  if (!live) {                                 // ไม่มีบอลเตะ → ข้าม grade (แต่ resync/auto-add ยังเช็กต่อ)
  ...
  if (doPeriodic) {
    ...
    try { await resyncKickoffs(ms48, espn); } catch(e){ console.log("⚠️ resync kickoff ล้มเหลว:", e.message); }   // แก้เวลาเพี้ยน/ตารางเลื่อน "ก่อน" ปิดรับ
