...
async function spiceOutro(frozen, kind, instr="") {
  if (!SPICE || !QWEN_TOKEN) return frozen;
  const prompt = `คุณคือบอท "AI กุ้ย-ชิน" ในกลุ่มไลน์เพื่อนสนิทเล่นทายผลบอลโลก 2026
...
- ❌ ห้ามแตะ/ห้ามพิมพ์บล็อกซ้ำ · ❌ ห้ามแซวแทรกทีละบรรทัด — เขียนแค่ "1-2 ประโยคสั้นๆ" จบในบรรทัดเดียว-สองบรรทัด ❌ ห้ามยาวเกินนั้น ❌ ห้ามมีคำนำ/ป้ายกำกับ/เครื่องหมาย \`\`\`
...
${frozen}`;
  try {
    const outro = cleanSpice(await dsChat(prompt, 160), "");
...
async function autoAddNext(ms) {   // ms = คู่ 48 ชม.ที่อ่านมาแล้ว (แชร์กับ nightDigest กัน read ซ้ำ)
  ...
  if (added && LINE_TOKEN && LINE_GROUP) {
    const list = addedList.map(x=>`• ${x.home} vs ${x.away} · ${thKick(x.ms)} น.`).join("\n");
    ...
    const instr = `- คัดมาแค่ 1-2 "คู่ไฮไลท์" ที่น่าสนใจ (ทีมใหญ่เจอทีมรอง/คู่เด็ด/คู่ขนาดต่างกันมาก) มาแซว แล้วกวนให้เพื่อนรีบทายก่อนปิด — ไม่ต้องพูดถึงทุกคู่
...
function formatLockMsg(m, preds) {
  ...
  return `🔒 ปิดรับ — ${m.home} vs ${m.away}\n${rows||"(ยังไม่มีใครส่งโพย)"}`;
}
...
async function nightDigest(recent) {   // recent = คู่ 48 ชม. (แชร์จาก autoAddNext กัน read ซ้ำ)
  ...
  const noZero = zeros.length
    ? `- คนที่ +0 (${zeros.join(", ")}) แซว "ไม่ขยับ/นิ่ง" ได้เต็มที่`
    : `- ❌ ทุกคนได้แต้มวันนี้ — ห้ามใช้คำ "ไม่ขยับ/ไม่ขยับเขยื้อน/แป้ก/นิ่ง/เงียบ/หายไป/+0" เด็ดขาด (ทำให้เข้าใจผิดว่ามีคนได้ 0) · จะแซวคนได้น้อยสุดให้ใช้ "ได้น้อยสุด/เก็บน้อย/รูดต่ำสุด/แต้มจิ๊บจ๊อย" แทน`;
  const instr = `- เลือกแซวแค่ 2-3 คนที่เด่น ยึด "ข้อเท็จจริงรอบนี้" นี้เป๊ะ ห้ามขัด: ${hint}
${noZero}`;
