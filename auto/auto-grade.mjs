...
async function spiceOutro(frozen, kind) {
  if (!SPICE || !QWEN_TOKEN) return frozen;
  const prompt = `คุณคือบอท "AI กุ้ย-ชิน" ในกลุ่มไลน์เพื่อนสนิทเล่นทายผลบอลโลก 2026
...
กฎ:
- เลือกแซวแค่ 2-3 คนที่เด่นพอ เช่น คนที่ได้แต้มวันนี้เยอะ=มาแรง / คนที่ได้ +0 ไม่ขยับ=แป้ก / คนนำหรือคนรั้งท้าย — ไม่ต้องพูดถึงทุกคน
- อ้างชื่อ/ตัวเลขได้เฉพาะที่อยู่ในตารางเท่านั้น ❌ ห้ามแต่งเลขใหม่ ❌ ห้ามสมมุติชื่อที่ไม่มี
- ❌ ห้ามแตะ/ห้ามพิมพ์ตารางหรือผลซ้ำ — เขียนแค่ย่อหน้าแซวล้วนๆ 2-4 บรรทัด ❌ ห้ามมีคำนำ/ป้ายกำกับ/เครื่องหมาย \`\`\`
ประเภท: ${kind}
...
async function nightDigest(recent) {   // recent = คู่ 48 ชม. (แชร์จาก autoAddNext กัน read ซ้ำ)
  ...
  const text = await spiceOutro(dry, "สรุปผลคู่ที่จบวันนี้ + ตารางคะแนน · เลขในวงเล็บ(+N)=แต้มที่ได้วันนี้ ใช้แซวว่าใครมาแรง(ได้แต้มเยอะ) ใครแป้ก(+0 ไม่ขยับ)");
