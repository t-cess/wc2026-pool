...
export function renderAdmin(){
  ...
  const memberRows=rosterNames().map(n=>{ const p=S.playersByName[n]; const claimed=!!p; const admE=adminEmailOf(n);
    ...
    return `<div style="display:flex;align-items:center;gap:9px;background:#14171D;border:1px solid #232830;border-radius:12px;padding:9px 6px 9px 12px;margin-bottom:7px;">${claimed?avatarHTML(p.photo,34):silhouetteHTML(34)}<div style="flex:1;min-width:0;"><div class="k" style="font-weight:700;">${esc(n)}${admBadge}</div><div style="font-size:11px;color:var(--mut);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${claimed?esc(S.emailByUid[p.uid]||"—"):"ยังไม่ล็อกอิน"} · ยกมา ${S.carry[n]||0}</div></div><div data-menu="${esc(n)}" class="k" style="cursor:pointer;flex:none;font-size:22px;color:#8A929E;padding:0 9px;line-height:1;">⋮</div></div>`; }).join("")||`<div class="k" style="color:var(--dim);">ยังไม่มีสมาชิก</div>`;
  box.innerHTML=`
    ...
    <div style="background:#14171D;border:1px solid #232830;border-radius:16px;padding:15px;">
      <div class="k" style="font-weight:700;font-size:15px;margin-bottom:12px;">🧑‍🤝‍🧑 สมาชิก (${rosterNames().length}) <span style="font-size:11px;color:#5b626d;">· แตะ ⋮ จัดการ</span></div>${memberRows}
