...
import { db, doc, setDoc, updateDoc, deleteDoc } from "./firebase.js";
import { TEAMS, fe } from "./config.js";
...
async function commitScorers(matchId){   // เขียน scorerOk ที่ติ๊กค้างไว้ ลง DB
  ...
  for(const p of preds){ const pid=`${p.matchId}__${p.uid}`;
    if(pid in S.scorerStage){ const v=S.scorerStage[pid];
      if(v!==!!p.scorerOk){ try{ await setDoc(doc(db,"predictions",pid),{scorerOk:v},{merge:true}); }catch(e){ toast("คนยิงบันทึกไม่ได้ (Rules?)"); } }
...
export function renderAdmin(){
  ...
  box.innerHTML=`
    ...
    <div style="background:#1a1410;border:1px solid #3a2f1e;border-radius:16px;padding:15px;margin-bottom:13px;">
      ...
      <div style="display:flex;gap:8px;"><div id="amSetChamp" class="k btnG" style="flex:1;height:42px;font-size:14px;background:var(--gold);color:#1a1410;">ตั้งแชมป์ (+10)</div><div id="amLockPicks" class="k" style="height:42px;display:flex;align-items:center;justify-content:center;padding:0 14px;border-radius:11px;border:1px solid #3a2f1e;color:${S.tournament.picksLocked?"#5fcf94":"#caa75a"};font-weight:700;font-size:13px;cursor:pointer;">${S.tournament.picksLocked?"🔒 ล็อกแล้ว":"ล็อกทายแชมป์"}</div></div></div>
    <div style="background:#14171D;border:1px solid #232830;border-radius:16px;padding:15px;margin-bottom:13px;">
  ...
  box.querySelectorAll("[data-delmem]").forEach(el=>el.onclick=async()=>{ const n=el.dataset.delmem; if(!confirm(`ลบสมาชิก "${n}"? (ลบคะแนนยกมา + ปลดการจับคู่)`))return;
    const c2={...S.carry}; delete c2[n]; await setDoc(doc(db,"config","carry"),c2);  // ไม่ merge = ลบ key
    const p=S.playersByName[n]; if(p&&p.uid){ try{ await deleteDoc(doc(db,"players",p.uid)); }catch(e){} }
    delete S.carry[n]; delete S.playersByName[n];
    toast("ลบสมาชิกแล้ว"); renderAdmin(); });
  if($("#amMemAdd")) $("#amMemAdd").onclick=async()=>{ const n=$("#amMemName").value.trim(); if(!n){toast("ใส่ชื่อ");return;} const v=parseInt($("#amMemCarry").value)||0; await setDoc(doc(db,"config","carry"),{...S.carry,[n]:v},{merge:true}); S.carry[n]=v; toast("เพิ่มสมาชิกแล้ว ✓"); renderAdmin(); };
  if($("#acEdit")) $("#acEdit").onclick=()=>{ S.carryEdit=true; renderAdmin(); };
  if($("#acSave")) $("#acSave").onclick=async()=>{ if(!confirm("บันทึกคะแนนยกมาใหม่? (กระทบยอดรวมทุกคน)"))return; const c2={...(S.carry||{})}; box.querySelectorAll("[data-carry]").forEach(i=>{c2[i.dataset.carry]=parseInt(i.value)||0;}); await setDoc(doc(db,"config","carry"),c2,{merge:true}); Object.assign(S.carry,c2); S.carryEdit=false; toast("บันทึกคะแนนยกมาแล้ว"); renderAdmin(); };
  ...
  $("#amSetChamp").onclick=async()=>{ const c=$("#amChampion").value.trim(); if(c&&!confirm(`ตั้งแชมป์ ${c}? (+10 ให้คนทายถูก)`))return; S.tournament.champion=c; S.tournament.championLocked=!!c; await setDoc(doc(db,"config","tournament"),{champion:c,championLocked:!!c},{merge:true}); toast("ตั้งแชมป์แล้ว 🏆"); renderAdmin(); };
  $("#amLockPicks").onclick=async()=>{ const nv=!S.tournament.picksLocked; S.tournament.picksLocked=nv; await setDoc(doc(db,"config","tournament"),{picksLocked:nv},{merge:true}); toast(nv?"ล็อกแล้ว":"ปลดล็อกแล้ว"); renderAdmin(); };
  $("#amSaveBatch").onclick=async()=>{ await setDoc(doc(db,"config","tournament"),{batchLabel:$("#amBatch").value.trim()},{merge:true}); toast("บันทึกป้ายแล้ว"); };
  bindAvatars(box);
}
