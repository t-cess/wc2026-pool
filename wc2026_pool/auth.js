...
export function enterAppUI(){   // เข้าแอป (แสดงผล) — ไม่ต่อ Firestore (mock ใช้ร่วม)
  ...
  if(!MOCK){ try{ const t=localStorage.getItem("wc_tab"); if(t && ["fixtures","champion","board","admin","manage"].includes(t) && (t!=="admin"||isAdmin()) && (t!=="manage"||isSuper())) S.tab=t; }catch(e){} }
  if(!S.me.name) S.tab="admin";   // แอดมินล้วน → เปิดแท็บแอดมิน
  renderNav();
  ["fixtures","champion","board","admin","manage"].forEach(t=>{ const el=$("#tab-"+t); if(el) el.classList.toggle("hidden",t!==S.tab); });
  if(S.tab==="admin" && isAdmin()) renderAdmin();
  if(S.tab==="manage" && isSuper()) renderManage();
}
